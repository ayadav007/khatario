...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ settings: await getHrExitSettings(businessId) });
  } catch (error) {
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const settings = await saveHrExitSettings(
      businessId,
      parseHrExitSettings({ ...(await getHrExitSettings(businessId)), ...body }),
    );
