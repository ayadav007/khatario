...
export const POST = withPremiumSubscriptionApi(
  ...
  async ({ body, businessId }) => {
    ...
    try {
      ...
      if (!supplier_id || !financial_year || !quarter || !issue_date) {
        return NextResponse.json(
          { error: 'supplier_id, financial_year, quarter, and issue_date are required' },
          { status: 400 }
        );
      }
    ...
    `, [businessId, supplier_id, financial_year, quarter]);
      ...
      const certNumber = `TDS/${financial_year}/${quarter}/${String(supplier_id).substring(0, 8).toUpperCase()}`;
