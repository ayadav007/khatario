 ...
 * Get purchase summary report for a date range
 */
export async function GET(request: NextRequest) {
  try {
    ...
    const summary = await db.queryOne(`
      ...
      FROM purchases
      WHERE business_id = $1 AND status != 'cancelled' AND deleted_at IS NULL ${dateFilter}
    `, queryParams);

    // Purchases by supplier
    const bySupplier = await db.queryRows(`
      SELECT 
        s.id, s.name,
        COUNT(p.id) as purchase_count,
        COALESCE(SUM(p.grand_total), 0) as total_amount
      FROM suppliers s
      INNER JOIN purchases p ON s.id = p.supplier_id AND p.deleted_at IS NULL
      WHERE p.business_id = $1 AND p.status != 'cancelled' ${dateFilter}
