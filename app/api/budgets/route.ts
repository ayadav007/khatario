...
export const POST = withPremiumSubscriptionApi(
  ...
  async ({ body, businessId }) => {
    ...
    try {
      ...
      if (!budget_name || !budget_type || !financial_year ||
          !period_start_date || !period_end_date || !lines || !Array.isArray(lines)) {
        return NextResponse.json(
          { error: 'budget_name, budget_type, financial_year, period_start_date, period_end_date, and lines are required' },
          { status: 400 }
        );
      }
      ...
      const existing = await queryOne(
        ...
        [businessId, budget_name, financial_year]
      );
