...
export async function GET(
  ...
  try {
    ...
    // 2. Transform for template (re-use number to words from presenter)
    // We use a dummy document structure to satisfy prepareInvoiceForRendering or just manually format
    const formatCurrency = (val: any) => Number(val || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 });
    
    // Internal number to words (simple version or import if possible)
    // Since we already have it in lib/invoice-presenter.ts, let's just use it by passing dummy data
