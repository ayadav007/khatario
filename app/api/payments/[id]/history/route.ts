 ...
 * Activity log for this payment (module = payments).
 */
export async function GET(
  ...
  try {
    ...
    const payment = await queryOne<{ id: string; business_id: string }>(
      'SELECT id, business_id FROM payments WHERE id = $1 AND business_id = $2 AND deleted_at IS NULL',
    ...
    const rows = await queryRows<ActivityLogRow>(
      ...
      WHERE al.business_id = $1
        AND al.module = 'payments'
        AND al.entity_id = $2
      ORDER BY al.created_at DESC
      LIMIT $3
      `,
      [payment.business_id, paymentId, limit]
    );
