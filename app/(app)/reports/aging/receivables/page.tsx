...
function ReceivablesAgingPage() {
  ...
  const fetchAging = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/reports/aging/receivables?${params}`);
      ...
      if (res.ok) {
        ...
        const errorData = await res.json().catch(() => ({ error: 'Failed to fetch receivables aging report' }));
        ...
        if (res.status === 403 || res.status === 401) {
          ...
          // Other errors
          setError({
            message: errorData.message || errorData.error || 'Failed to fetch receivables aging report',
      ...
      setError({
        message: 'Failed to fetch receivables aging report',
        code: 'NETWORK_ERROR'
      });
    } finally {
      setLoading(false);
  ...
  return (
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-text-primary">Receivables Aging Report</h1>
            <p className="text-sm text-text-secondary mt-1">Outstanding receivables by age</p>
          </div>
        ...
        <Card>
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-text-secondary">No receivables found</p>
            </div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody>
                  {data.map((item) => (
                    <tr key={item.customer_id} className="border-b border-border hover:bg-gray-50">
                      <td className="py-4 px-4 font-medium">{item.customer_name}</td>
