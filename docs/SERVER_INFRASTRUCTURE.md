...
| Platform Meta Cloud API | Staging templates: `META_WA_*` below. Do **not** copy tokens to production until a test send works on staging |
...
Khatario’s **platform** WABA (help@ number) is managed in `/admin` → **WhatsApp templates**. Tenant Baileys (QR inbox, campaigns, invoice WhatsApp) is unchanged.

Add these to staging `/var/www/khatario/.env.production` only. Never commit them. Leave them empty on production until a staging test send succeeds.

| Env | Purpose |
|---|---|
| `META_WA_ACCESS_TOKEN` | System user token (`whatsapp_business_management` + `whatsapp_business_messaging`). Server only |
| `META_WA_WABA_ID` | WhatsApp Business Account ID |
| `META_WA_PHONE_NUMBER_ID` | Cloud API send-from phone number ID |
| `META_WA_APP_SECRET` | Verifies `X-Hub-Signature-256` on status webhooks |
| `META_WA_VERIFY_TOKEN` | Arbitrary string for Meta’s webhook handshake |

Webhook (Cloudflare → staging nginx → Next):

`https://staging.khatario.com/api/webhooks/meta-whatsapp`

In Meta App Dashboard subscribe to **`message_template_status_update`**. After `git pull` + `bash scripts/deploy-vps.sh`, migration `290_platform_whatsapp_templates.sql` runs with other pending migrations.
