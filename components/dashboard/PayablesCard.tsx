...
export const PayablesCard: React.FC<PayablesCardProps> = ({ total, aging }) => {
    ...
    <Card padding="md" className="border border-border">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          <h3 className="text-lg font-semibold text-text-primary">Total Payables</h3>
          <div className="relative group">
            ...
            <div className="absolute left-0 bottom-full mb-2 w-64 p-3 bg-gray-900 text-white text-xs rounded-lg shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-10">
              <p className="font-semibold mb-2">Total Payables:</p>
              <ul className="space-y-1 list-disc list-inside">
                <li><strong>Current:</strong> Bills not yet due (within payment terms)</li>
                <li><strong>Overdue:</strong> Bills past their due date, broken down by days overdue</li>
                <li>Click on any section to view filtered bills</li>
              </ul>
      ...
      <p className="text-sm text-text-secondary mb-4">
        Total Unpaid Bills ₹{total.toLocaleString('en-IN', { maximumFractionDigits: 2 })}
      </p>

      <div className="space-y-3">
        {/* Current (Not Overdue) */}
        <Link 
          href="/purchases?status=unpaid&aging_days_min=0&aging_days_max=0"
        ...
        <div>
          ...
          {showAging && (
            <div className="mt-2 space-y-2 pl-4 border-l-2 border-red-200 dark:border-red-900 animate-in fade-in slide-in-from-top-2 duration-200">
              <Link 
                href={`/purchases?status=unpaid&aging_days_min=1&aging_days_max=15`}
              ...
              <Link 
                href={`/purchases?status=unpaid&aging_days_min=16&aging_days_max=30`}
                className="flex items-center justify-between py-2 hover:bg-red-50 dark:hover:bg-red-950/25 rounded px-2 -mx-2 transition-colors cursor-pointer"
              >
              ...
              <Link 
                href={`/purchases?status=unpaid&aging_days_min=31&aging_days_max=45`}
                className="flex items-center justify-between py-2 hover:bg-red-50 dark:hover:bg-red-950/25 rounded px-2 -mx-2 transition-colors cursor-pointer"
              >
              ...
              <Link 
                href={`/purchases?status=unpaid&aging_days_min=46&aging_days_max=9999`}
                className="flex items-center justify-between py-2 hover:bg-red-50 dark:hover:bg-red-950/25 rounded px-2 -mx-2 transition-colors cursor-pointer"
              >
